...
| Navbar | `Navbar.tsx`, `nav-menu-config.ts` | Logo C, Aktualności, działy (w tym **Rozrywka**), Kategorie, Więcej (Odkrywaj), Szukaj, Powiadomienia, Konto |
...
| 5–7 (v2) | Technologie, Astronomia, **Rozrywka** | split-lead homepage v2 | Dobrze |
| 5–9 (legacy) | Technologie, Astronomia, Misje, Ziemia, ISS | po 4 artykuły z DB | Dobrze |
...
| `/technologie` | `app/technologie/page.tsx` | j.w. — starty, rakiety, infrastruktura (bez gamingu) |
| `/rozrywka` | `app/rozrywka/page.tsx` | **Dobrze** — gaming, filmy/sci-fi; feed `category=rozrywka` |
...
| `/sitemap.xml` | `app/sitemap.ts` + `lib/seo/public-routes.ts` | **Dobrze** — statyczne trasy z `SEO_SITEMAP_PATHS` (w tym **`/rozrywka`**) + wszystkie `PUBLISHED` → `/aktualnosci/[slug]` |
