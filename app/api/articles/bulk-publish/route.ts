...
import { bulkPublishArticles } from "@/lib/server/bulk-articles";
...
export async function POST(request: NextRequest) {
  try {
    const guard = await requirePermission(canPublishArticle);
    ...
    const result = await bulkPublishArticles(ids);
    return NextResponse.json({ data: result });
  } catch (err) {
    console.error("[POST /api/articles/bulk-publish]", err);
    return jsonError(500, "INTERNAL_ERROR", "Failed to bulk publish articles.");
  }
}
