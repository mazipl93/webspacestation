  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900/60 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2">
          {extremeCount > 0 && (
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-red-900/50 text-red-300 border border-red-700/50 animate-pulse">
              {extremeCount} DELTA
            </span>
          )}
          {highCount > 0 && !extremeCount && (
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-orange-900/50 text-orange-300 border border-orange-700/50">
              {highCount} GAMMA
            </span>
          )}
        </div>
      ...
      <div className="p-3 border-b border-slate-800/60">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] text-slate-500 font-mono">Obraz SDO — NASA</span>
        ...
        <a href={fullSrc} target="_blank" rel="noopener noreferrer" className="block group relative">
          <div className="relative w-full aspect-square max-h-52 overflow-hidden rounded border border-slate-700/40 bg-black flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex items-end justify-end p-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <span className="text-[9px] font-mono bg-black/70 text-slate-300 px-1.5 py-0.5 rounded">
                powieksz
              </span>
            </div>
          </div>
        </a>
        <p className="text-[9px] text-slate-600 font-mono mt-1 text-center">
      ...
      <div className="overflow-auto max-h-48">
        {sorted.length === 0 ? (
          <div className="px-3 py-4 text-center text-[10px] text-slate-600 font-mono">
          ...
          <table className="w-full text-[10px] font-mono">
            <thead className="sticky top-0 z-10" style={{ background: "#0d1117" }}>
              <tr className="border-b border-slate-800/60">
                <th className="px-3 py-1.5 text-left text-slate-500 font-normal">AR</th>
                <th className="px-2 py-1.5 text-left text-slate-500 font-normal">Lokalizacja</th>
                <th className="px-2 py-1.5 text-left text-slate-500 font-normal">Klasa</th>
                <th className="px-2 py-1.5 text-right text-slate-500 font-normal">Plamy</th>
                <th className="px-2 py-1.5 text-right text-slate-500 font-normal pr-3">MSH</th>
              </tr>
            </thead>
            <tbody>
              {sorted.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] ${style.badge}`}>
                        {r.mag_class}
                      </span>
                    </td>
      ...
      {/* Legend */}
      <div className="px-3 py-2 border-t border-slate-800/60 flex flex-wrap gap-x-3 gap-y-1">
